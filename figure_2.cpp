#include "figure_2.h"
#include "ui_figure_2.h"
#include "choice.h"

figure_2::figure_2(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::figure_2)
{
    ui->setupUi(this);
...
const int SAES_2::INVERSE_S_BOX[4][4] = {
        {0xA, 0x5, 0x9, 0xB},
        {0x1, 0x7, 0x8, 0xF},
        {0x6, 0x0, 0x2, 0x3},
        {0xC, 0x4, 0xD, 0xE}
...
const int SAES_2::MIX_COLUMNS_MATRIX[2][2] = {
        {0x1, 0x4},
        {0x4, 0x1}
};

const int SAES_2::INVERSE_MIX_COLUMNS_MATRIX[2][2] = {
        {0x9, 0x2},
        {0x2, 0x9}
};

void figure_2::on_pushButton_clicked()
{
    ...
    if(text1.length()==0)
    {
        ...
        else
        {
            ...
            if(w)
            {
                string key=text2.toStdString();
                bitset<16> keybits(key);
                string encrypted=text3.toStdString();
                bitset<16> encryptedBinary = SAES_2::asciiToBinary(encrypted);
                int encryptedword = static_cast<int>(encryptedBinary.to_ulong());
                int keyword = static_cast<int>(keybits.to_ulong());
                int decrypted = SAES_2::decrypt1(encryptedword, keyword);
                bitset<16> decryptedbits(decrypted);
                string decryptedAscii = SAES_2::binaryToAscii(decryptedbits);
    ...
    else
    {
        ...
        else
        {
            ...
            if(w)
            {
                ...
                bitset<16> plaintextBinary = SAES_2::asciiToBinary(plaintext);
                int plaintextword = static_cast<int>(plaintextBinary.to_ulong());
                int keyword = static_cast<int>(keybits.to_ulong());
                int encrypted = SAES_2::encrypt1(plaintextword, keyword);
                bitset<16> encryptedbits(encrypted);
                string encryptedAscii = SAES_2::binaryToAscii(encryptedbits);
...
void figure_2::mousePressEvent(QMouseEvent *e)
{
